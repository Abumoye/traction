...
has been replaced by a new four-field T-ICR 2026 registration form, plus a
new affiliate sign-up form. Those two use their **own separate** Google
Sheet and Apps Script project — see `docs/tcr-events-backend-setup.md` for
that setup. `js/retreat-registration.js` has been removed from the project;
if you previously set up a **Kigali Retreat 2026** sheet tab for it, it can
stay in place for historical records, but the site no longer submits to it.
